...
/* ═══ ИМЕНА ВКЛАДОК — ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ОНИ ЗАДАЮТСЯ ═══════════
   ...
   при переименовании пропустили. Теперь место одно.
...
var ЛИСТЫ = {
  ...
  предзапись:  'Предзапись',            // заявки из приложения теста
...
function проверитьЛисты() {
  ...
  Object.keys(ЛИСТЫ).forEach(function (ключ) {
    var имя = ЛИСТЫ[ключ];
    var sheet = ss.getSheetByName(имя);
    строки.push((sheet ? '✔ ' : '✘ ') + ключ + ': «' + имя + '»'
                + (sheet ? ' — строк: ' + Math.max(0, sheet.getLastRow() - 1)
                         : ' — ВКЛАДКИ НЕТ, будет создана заново'));
  });
