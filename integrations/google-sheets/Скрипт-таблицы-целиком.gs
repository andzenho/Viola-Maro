...
/* Куда какая форма пишет. «Заявка» и «оплата» делят один лист намеренно:
   команде удобнее видеть всех, кто выбрал тариф, в одном месте, а отличает
   их колонка «Готовность» — у заявки там «Заявка с канала, без оплаты». */
var SITE_SHEETS = {
...
function route_(d) {
  try {
    ...
    /* Всё, что ниже, — результат теста эмпата. Если пришла форма, которой
       этот скрипт ещё не знает (сайт обновили, а скрипт нет), дописывать её
       в «Тест эмпата» нельзя: имя и телефон в этот лист не попадают вовсе,
       строка ляжет пустой, а сайт покажет человеку «заявка принята».
       Честная ошибка лучше: она видна сразу и на сайте, и здесь. */
...
function saveSite_(d) {
  ...
  // Оферту принимают только там, где платят. На предзаписи и на странице
  // заявки покупки нет, значит нет и акцепта: требовать согласие с договором
  // купли-продажи там, где ничего не покупают, юридически неверно.
  // Согласие на обработку данных нужно всегда — контакты мы собираем везде.
