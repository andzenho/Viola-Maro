...
# Кадр надставляется с двух сторон. Экран вдвое уже, чем высок, поэтому
# вертикальный кусок вписывается по высоте, а лишнее срезается слева.
# Чем кадр выше, тем меньше масштаб и тем шире полоса исходника входит
# в экран — значит, тем левее можно начать кроп и тем правее встанет Виола.
# Надставка сверху опускает портрет, надставка снизу поднимает.
MOBILE_CROP_X = 470      # левее портрета — полоса пустого фона под текст
MOBILE_EXT_TOP = 300     # опускает портрет: верх головы на 22% высоты
MOBILE_EXT_BOTTOM = 560  # поднимает и расширяет; уходит под вуаль и кнопки
...
def build_images():
    ...
    mob.paste(top, (0, 0))
    mob.paste(crop, (0, MOBILE_EXT_TOP))
    mob.paste(bottom, (0, MOBILE_EXT_TOP + H))
