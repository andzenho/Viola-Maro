...
# Телефонный кадр — просто вырезка из снимка, без единого дорисованного
# пикселя. Раньше кадр надставлялся сверху и снизу растянутыми строками:
# это давало точный контроль над положением портрета, но низ читался как
# размазанные полосы. Теперь положение задаёт сама фотография.
#
# Вырезка идёт от x=887 до правого края. Левее нельзя: при высокой рамке
# кадр вписывается по высоте, в экран попадает всего 463 px исходника,
# и начни вырезку левее — правый край срежет лицо.
MOBILE_CROP_X = 887
